...
class TestAttendanceService:
    ...
    def test_register_attendance_event_not_found(self, db, create_participant):
        ...
        service = AttendanceService(db)
        attendance_data = AttendanceCreate(
            event_id=999, participant_id=create_participant.id
        )
    ...
    def test_register_attendance_participant_not_found(self, db, create_event):
        ...
        # Arrange
        service = AttendanceService(db)
        attendance_data = AttendanceCreate(event_id=create_event.id, participant_id=999)
    ...
    def test_register_attendance_capacity_exceeded(
        self, db, create_event, create_multiple_participants
    ):
        ...
        attendance_data = AttendanceCreate(
            event_id=create_event.id, participant_id=create_multiple_participants[2].id
        )
    ...
    def test_cancel_attendance(self, db, create_attendance):
        ...
        assert (
            db.query(Attendance).filter(Attendance.id == attendance_id).first() is None
        )
    ...
    def test_get_participant_attendances(self, db, create_attendance):
        ...
        # Act
        attendances = service.get_participant_attendances(
            create_attendance.participant_id
        )
